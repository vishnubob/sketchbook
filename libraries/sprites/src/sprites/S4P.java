...
public class S4P implements PConstants {
	...
	/**
	 * If you want to see the collision area for the sprites then set to tru.<br>
	 * This should only be done when testing collision detection because it 
	 * uses a lot of memory creatin images for pixel-pixel collision areas
	...
	/** 
	 * This is a read only attribute and represents the time between frames.
	 */
	 ...
	 */
	protected static float worldX = 0.0f;

	/**
	 * World coordinates corresponding to top-left of display
	 */
	protected static float worldY = 0.0f;
	 ...
	 * @param scale
	 */
	public static void resizeWorld(float scale){
		float newX, newY;
	...
	/**
	 * Reset the worl so that the world origin is at the top-left corner
	 ...
	 */
	public static PointF2D pixel2world(float px, float py){
		PointF2D w = new PointF2D();
	 ...
	 */
	public static Point world2pixel(float wx, float wy){
		Point p = new Point();
	...
	/**
	 * Calculate the time (seconds) since this method was last called. Call
	 * this just before you call the updateSprites() method.
	 */
	...
	public static void drawSprites(){
		app.pushMatrix();
		app.scale(worldScale);
		app.translate(-worldX, -worldY);
		Iterator<Sprite> iter = sprites.iterator();
